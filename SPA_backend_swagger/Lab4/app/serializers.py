...
class ConsumersSerializer(serializers.ModelSerializer):
    class Meta:
        model = Consumer
        fields = ("id", "name", "status", "power", "category")
...
class ConsumerAddSerializer(serializers.ModelSerializer):
    class Meta:
        model = Consumer
        fields = ("name", "description", "power", "image")


class ReserveBaseSerializer(serializers.ModelSerializer):
...
class ReservesSerializer(ReserveBaseSerializer):
    consumers_count = serializers.SerializerMethodField()

    def get_consumers_count(self, reserve):
        items = reserve.consumerreserve_set.all()
        return items.count()


class ReserveSerializer(ReserveBaseSerializer):
...
class ConsumerItemSerializer(ConsumerSerializer):
    ...
    class Meta:
        model = Consumer
        fields = ("id", "name", "status", "power", "category", "percentage")
